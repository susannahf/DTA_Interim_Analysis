...
It appears that the two limits converge closer to p0 as both alpha and n increase, and that the H0 limit at n/2 is approximately p0.
...
* upload onto Github (DTA_Interim_Analysis on personal acct)
...
csplot <- ggplot(data=csboth, aes(x=x, y=Sespvalue, colour=Sesp, label=Sesp)) + 
  geom_line() +
  theme_bw() +
  geom_text_repel(data=subset(csboth, x==200), nudge_x = 30) +
  scale_color_grey() +
...
threshplot <- ggplot(data=threshboth, aes(x=x, y=thvalue, colour=Measure, label=Measure)) + 
  geom_line() +
  theme_bw() +
  geom_text_repel(data=subset(threshboth, x==50), nudge_x = -30) +
  scale_color_grey() +
  theme(legend.position="none") +
  labs(x="Data point", y="Proportion") +
  facet_grid(. ~ source)
